...
document.addEventListener("DOMContentLoaded", function() {
  document.documentElement.setAttribute("data-build", window.VERAXA_CONFIG?.build || "missing-config");
...
function veraxaMoney(value) {
  return (window.VERAXA_CONFIG?.currency || "₺") + " " + Number(value).toLocaleString("tr-TR");
}
...
function veraxaGetFilteredProducts() {
  ...
  return products.filter(function(product) {
    const filterOk =
      ...
      product.collection?.toLowerCase() === veraxaCurrentFilter ||
    ...
    const searchOk =
      ...
      product.colorText.toLowerCase().includes(query);
...
function veraxaRenderFeaturedCategories() {
  ...
  container.innerHTML = window.VERAXA_CATEGORIES.map(function(category) {
    return (
      '<button type="button" onclick="veraxaSetFilter(\\'' + category.id + '\\', this)">' +
      ...
      '<article class="product-card">' +
        ...
        '<div class="product-info">' +
          ...
          '<div class="product-actions">' +
            ...
            '<button type="button" onclick="veraxaToggleWishlist(\\'' + product.id + '\\')">' + (isFavorite ? "Favoriden Çıkar" : "Favori") + '</button>' +
          '</div>' +
        '</div>' +
      '</article>'
    );
...
function veraxaOpenProduct(index) {
  ...
  document.getElementById("modalWhatsapp").onclick = function() {
    ...
    const message =
      "Merhaba VÉRAXA, ürün hakkında bilgi almak istiyorum:%0A%0A" +
      "Ürün: " + product.name + "%0A" +
      "Fiyat: " + veraxaMoney(product.price) + "%0A" +
      "Beden: " + size;
